...
export default function ShareDialog({ open, onClose, timer, onTimerUpdated }: ShareDialogProps) {
  ...
  return (
    <ModalBackdrop open={open} onClose={onClose} maxWidth="max-w-sm">
      <h2 className="mb-4 text-center text-lg font-bold text-app-fg">Share Timer</h2>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-sm text-app-muted">Make Public</span>
          <button type="button" onClick={togglePublic} disabled={toggling} className="relative h-6 w-11 rounded-full transition" style={{ backgroundColor: timer.isPublic ? theme.primary : "var(--app-toggle-off)" }} role="switch" aria-checked={timer.isPublic}>
            <span className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-app-surface-strong transition-transform ${timer.isPublic ? "translate-x-5" : ""}`} />
          </button>
        </div>
        {timer.isPublic && (
          <div>
            ...
            <div className="flex items-center gap-2">
              <input type="text" value={shareUrl} readOnly className="flex-1 rounded-lg border border-app-border bg-app-input px-3 py-2 text-xs text-app-fg outline-none" />
      ...
      <button type="button" onClick={onClose} className="mt-4 w-full rounded-xl border border-app-border py-2 text-sm text-app-muted hover:text-app-fg">Close</button>
    </ModalBackdrop>
  );
}
